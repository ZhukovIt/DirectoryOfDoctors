...
{
    public partial class SpecializationsList : Form
    {
        ...
        public SpecializationsList(Main mainWindow)
        {
            InitializeComponent();
            MainWindow = mainWindow;
            connectionString = ConnectionString.GetDirectoryOfDoctorsConnectionString();
            sqlOrder = "SELECT s.id as Номер, s.specialization as Специальность FROM specializations as s;";
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                adapter.InsertCommand = new SqlCommand("sp_CreateSpecialization", connection);
                adapter.InsertCommand.CommandType = CommandType.StoredProcedure;
                adapter.InsertCommand.Parameters.Add(new SqlParameter("@id", SqlDbType.Int, 0, "Номер"));
                adapter.InsertCommand.Parameters.Add(new SqlParameter("@specialization", SqlDbType.NVarChar, 100, "Специальность"));
